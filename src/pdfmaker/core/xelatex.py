...
旧实现直接 ``subprocess.run([xelatex, ...])`` 不带 timeout：一旦 xelatex 因字体缺失 /
宏包冲突 / 死循环宏陷入挂起，单章 ``chapter`` 编排与整书 ``build`` 合并都会无限卡住，
既不报错也不退出，作者只能手动 kill。

本封装把超时逻辑收口到一处：
- 超时时长取自 ``cfg.XELATEX_TIMEOUT``（默认 300s，可被 .pdfmaker.toml 的
  ``xelatex_timeout`` 或环境变量 ``PDFMAKER_XELATEX_TIMEOUT`` 覆盖，运行时经
  ``cfg.X`` 动态读取，保证覆盖生效）。
- 超时时不抛 ``TimeoutExpired`` 向上冒泡（避免吓人的栈回溯），而是杀掉进程并以约定的
...
def compile(xelatex: str, *tex_args: str, cwd: str, timeout: "float | None" = None) -> int:
