...
1. **整书初始化**（旧用法，向后兼容）
   ``python -m pdfmaker scaffold <dir> [--chapters N]``
   在 ``<dir>`` 落地 ``main.tex`` / ``_tmp.tex`` 与若干 ``第N章/第N章.tex`` 占位。

2. **单章从素材生成**（#9 治本能力，新增）
   ``python -m pdfmaker scaffold chapter N [--material <素材.md>] [--root <项目根>] [--force]``
   ...
   - 内嵌**可直接编译**的合规示例：booktabs 三线表（表题在上）、TikZ 矢量图
     ...
     消灭第 10–12 章那种"凭记忆写、退化成 ``\hline`` 网格表 + 裸 ``verbatim``"的漂移；
   - 按素材二级标题预生成 ``\section``，作者不必重新规划结构；
   - 按素材 URL 预填 ``\bibitem{cNrK}\href{URL}{...}`` 文献条目，并在引言处给出引用
     示例，保证 ``bibitem == cite == 链接数`` 开箱一致（不触发孤儿文献门禁）。

   该骨架经 `chapter.py` 标准 SOP 在章文件缺失且提供 `--material` 时**自动调用**
   （无需 `--scaffold` 显式开关），使整条写作流水线自我引导、不再依赖"从空白文件手写"。

设计原则：生成的文件一律**不覆盖**已有成果（``_write_if_absent``）；单章模式仅在
目标 ``第N章/第N章.tex`` 缺失时落地（除非 ``--force``）。
...
# 素材 URL 提取正则（与 reader.py 保持一致，避免双源漂移）。
...
def _compliance_examples(n: int) -> str:
    """内嵌可直接编译的合规示例：三线表（表题上）+ TikZ 图（图题下）+ codeblock。
...
def build_chapter_skeleton(n: int, *, title: str = "", sections: "list[str] | None" = None,
    ...
    if urls:
        ...
        bib_items.append(
            f"% \\bibitem{{c{n}r1}} 作者. 文献标题. \\emph{{来源}}, 年份. "
            f"\\href{{https://example.com/x}}{{示例链接}}"
        )
...
def _build_sections(n: int, secs: "list[str]", *, intro_lead: str = "") -> str:
    ...
    lead = (
        ...
        f"（如第~6--12 章）的关系、以及核心结论。{intro_lead}"
    )
    parts: list[str] = []
...
def write_chapter(n: int, *, material: "str | None" = None,
                  root: "str | None" = None, force: bool = False) -> bool:
    """生成单章骨架并写盘。返回是否实际写入。

    - 目标：``<root>/第N章/第N章.tex``（root 默认当前目录）。
    - 解析素材（标题/二级节/URL）预填骨架；不传素材则空白默认骨架。
    - 已存在且非 ``--force`` 时跳过（不覆盖）。
    """
    root_path = Path(root) if root else Path.cwd()
    chdir = root_path / f"第{n}章"
...
def _starter_chapter(n: int) -> str:
    """整书初始化用的单章占位（现升级为合规富骨架，从根上消除样式漂移）。"""
...
def _cmd_init(argv: list[str]) -> int:
    ...
    if target.exists() and any(target.iterdir()) and not args.force:
        ...
        print(f"          如需在现有目录中补齐缺失骨架（不覆盖已有文件），加 --force。")
        return 2
    ...
    for n in range(1, max(args.chapters, 0) + 1):
        chapters_list_lines.append(f"\\input{{第{n}章/第{n}章}}")
    for i in range(max(args.appendices, 0)):
        label = chr(ord("A") + i)
        chapters_list_lines.append(f"\\input{{附录{label}/附录{label}}}")
    ...
    print(f"  _tmp.tex")
    print(f"  正文 {max(args.chapters, 0)} 章（合规富骨架）、附录 {max(args.appendices, 0)} 个")
    if args.force:
        print(f"  （--force：已存在文件已跳过，未覆盖）")
    print(f"\n下一步：编辑各章 .tex 后，在 {target} 内运行 `python -m pdfmaker build .`")
    return 0


def _cmd_chapter(argv: list[str]) -> int:
    """单章从素材生成：``scaffold chapter N [--material <md>] [--root <dir>] [--force]``。"""
    parser = argparse.ArgumentParser(
        prog="pdfmaker scaffold chapter",
        description="从素材（可选）生成单章合规富骨架（第N章/第N章.tex）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例:\n"
            "  python -m pdfmaker scaffold chapter 14 --material 素材-第14章.md\n"
    ...
    args = parser.parse_args(argv)
    write_chapter(args.n, material=args.material, root=args.root, force=args.force)
    return 0
